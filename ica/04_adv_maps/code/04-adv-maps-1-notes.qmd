...
- Look through the examples section. What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function?
  ...
  > - **`_d()` = discrete** data (categories/groups), such as different cities.
  >   **`_c()` = continuous** data, where color represents a continuous range of values, such as temperature or density.
  >   **`_b()` = binned** data, where continuous values are divided into intervals/bins before colors are assigned.

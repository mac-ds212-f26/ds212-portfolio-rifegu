...
**Challenge:** Use <https://epsg.io> to **find two CRSs** that result in a North Carolina map that is noticeably different from the original in the NAD27 CRS. Below is a **function** that re-map a spatial object using a new CRS.
Use it after modifying it as follows:
...
transform_and_plot(nc, new_crs = "ESRI:102556" )
...
> Firstly: note to self; these systems (ESRI etc) are basically each countries reference to fit their own country in the map.
> I would expect the accuracy to of how they would be overlaid on the original North Caroline map to be low.
...
- Read the function description at the top.
  What is the advantage of using this function for making color palettes?

  > for people with color blindness.

- Look through the examples section.
  What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function?

  > - **`_d()` = discrete** data (categories/groups), such as different cities. **`_c()` = continuous** data, where color represents a continuous range of values, such as temperature or density. **`_b()` = binned** data, where continuous values are divided into intervals/bins before colors are assigned.
...
ggsave("relative path to image & file extension", width = ?, height = ?, units = "in")
```
